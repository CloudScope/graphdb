// Lab 4 setup — run this whole block in Neo4j Browser.
// Uses MERGE throughout so it's safe to re-run, and so it works even if
// Lab 3's data isn't already loaded (it'll create Alice/Bob/Carol/Dave here).
...
// A -> B -> C -> D: an ordinary chain of everyday transactions.
...
// D -> E: bridges the ordinary chain into the fraud ring below.
...
// E -> F: flagged fraudulent, made from dev-0003.
...
// F -> A: NOT flagged, but made from the SAME device (dev-0003) as the fraud
// transaction above — the pattern Lab 4's device-sharing query should surface,
// and it also closes a ring back to Alice for the shortest-path exercise.
